    ...
    def get_filter_action_count(self):
        ...
        for fc in range(self.col_fold):
            for fr in range(self.row_fold):
                ...
                filter_read_prefill_action_count = (row_used - 1) * col_used * self.T
    ...
    def get_ofmap_action_count(self):
        ...
        for fc in range(self.col_fold):
            for fr in range(self.row_fold):
                ...
                ofmap_write_postfill_action_count = (1 + self.arr_row) * self.arr_row * 0.5 * col_used - (1 + self.arr_row - row_used) * (self.arr_row - row_used) * 0.5 * col_used - row_used * col_used
                ofmap_write_action_count_fold = ofmap_write_compute_action_count + ofmap_write_postfill_action_count
                ofmap_read_compute_action_count = row_used * col_used * (self.T - 1)
                ofmap_read_postfill_action_count = (1 + self.arr_row) * self.arr_row * 0.5 * col_used - (1 + self.arr_row - row_used) * (self.arr_row - row_used) * 0.5 * col_used
